...
//    "too-large" instead of blocking. Combined with the analyze-loop yield
//    (a running analysis job no longer starves this path) and per-IP rate
//    limiting, this bounds the worst case to a short, rare block.
//
...
const DEFAULT_LIMITS: Required<PatchLimits> = {
  maxFileBytes: 256 * 1024, // 256 KB — a normal source file; a dense 256 KB blob parses in ~180ms
  maxTotalBytes: 512 * 1024, // 512 KB cumulative — bounds the worst-case block to ~350ms
  maxEntries: 200,
};
