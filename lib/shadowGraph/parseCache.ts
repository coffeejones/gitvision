...
export const ANALYZER_VERSION = "9"; // 9: Next.js App Router route handlers are declared entry points, so a JS/TS app finally has somewhere for reachability to start (8: taint cleared on reassignment, so a sanitiser is no longer reported as the vulnerability; 7: ~15 new sink rules + import aliases, await/request-method taint, receiver-position sinks; 6: Python entry points + security sinks; 5: Ruby parenless calls)
