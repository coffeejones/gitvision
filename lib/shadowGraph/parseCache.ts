...
export const ANALYZER_VERSION = "10"; // 10: Java types enum constants, enhanced-for variables and array elements as receivers, so a JUnit test finally reaches the method it asserts (9: Next.js App Router route handlers are declared entry points; 8: taint cleared on reassignment, so a sanitiser is no longer reported as the vulnerability; 7: ~15 new sink rules + import aliases, await/request-method taint, receiver-position sinks; 6: Python entry points + security sinks; 5: Ruby parenless calls)
