...
RepoJury is **the codebase intelligence layer for teams where AI
...
- **repojury.com workspace** (existing, polished) for deep-dive +
  cross-PR view
...
- repojury.com auth + per-user state
...
We need to enumerate explicitly which signals RepoJury has that
...
- `eval/recon/competitors-2026-05.md` — competitive landscape this
  plan responds to
- `eval/baseline/p1-p2-cells.json` — eval data underpinning "RepoJury
