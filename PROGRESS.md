...
- **Arc 4 "Evidence Desk"** — the security/supply-chain arc (Free = look, Plus = watch, **Pro = prove**), four beats:
  - **CI-hardening signals** (`lib/ciHardening/`, `135f04a`) — reads `.github/workflows/*.yml` during analysis; three deterministic detectors (unpinned actions, third-party inventory, GITHUB_TOKEN scope), each grounded in a named dated incident (tj-actions CVE-2025-30066). Panel on the Packages tab. Adds the `yaml` dep.
  - **Incident-exposure pages** (`/exposure` + `/exposure/[id]`, `91b758e`) — public, SSG'd supply-chain tracker over `KNOWN_INCIDENTS`; auto-grows as incidents are curated; the shareable "are you exposed to X?" page per named attack. Reuses the existing `findIncidentMatches` matcher.
  - **SBOM export** (`lib/sbom/`, Pro, `ad415b2`) — CycloneDX 1.5 + SPDX 2.3 from a newly-captured full component list (`DependencyHealth.components`, can't-backfill), purl-identified, Pro-gated download route. Adversarially reviewed — the review ran the real spdx-tools validator and caught an SPDX timestamp bug (millis) that failed validation; fixed + `filesAnalyzed`/`supplier` added.
  - **Evidence pack** (`lib/evidencePack/`, Pro, `0f31ac6`) — one timestamped zip (`fflate`): SBOMs + `evidence.json` + `SUMMARY.md` bundling dep-health, CI-hardening, secret-hygiene (**counts only, never values**), and the re-sweep trail. Completes the tier story.

**Forward plan lives in [`ROADMAP.md`](ROADMAP.md)** (web-grounded, H2 2026): five arcs — Refactor-Confidence (Arc 1, shipped), Agent Pre-Flight / MCP (Arc 2, shipped), Temporal drift (Arc 3, shipped), Evidence Desk / CRA (Arc 4, **shipped**), Change-Time Blast (Arc 5, **next**). Arcs 1–4 are all on `feat/codetrawl-landing`; not yet merged to `main`/prod.
...
- ✅ v0.31 — **GitHub App PR-bot end-to-end (Phase 4 wedge).** Native GitHub App that comments on every PR with the same deterministic signal layer as the workspace. Nine commits land the full chain: webhook receiver + HMAC-SHA256 signature verification (`lib/githubApp/webhook.ts`), JWT + installation-token auth wrapping Octokit's App class (`auth.ts`), zod-validated event dispatch with action / private-repo / draft / bot-author filters (`events/pullRequest.ts`), dep-injectable analysis pipeline orchestrating `analyzeRepo` × 2 + `computeDiff` + `evaluateVerificationRules` (`pipeline.ts`), markdown formatter with versioned `<!-- gitvision:pr-review v1 -->` marker for find-or-update (`comment.ts`), Octokit-based comment poster that paginates + matches marker + creates-or-updates (`poster.ts`), three cost guardrails — per-installation rate limit (10/hr via existing `lib/rateLimit.ts`), repo-size cap (100 MB), max-2 concurrent slot acquired/released around the pipeline (`guardrails.ts`), and `installation.deleted` GC that sweeps sessions tagged with the uninstalled installation id (`events/installation.ts` + new `deleteSessionsByInstallation` in `storage.ts`). Heavy work runs via Next 16's `after()` so the webhook response ships in <100ms regardless of analysis duration — decoupling GitHub's 10s response-timeout from our 30s-2min analysis runtime. Live-validated on a test PR against `coffeejones/repobaronTest`: full chain ran in <60s, "Nothing notable on this PR ✅" comment posted with installation auth, the empty-state path fired correctly because Δ=3 is below the calibrated Δ≥4 threshold for `complexity-increase-without-test`. Validation trail: [eval/strategy/github-app-validation-2026-05.md](./eval/strategy/github-app-validation-2026-05.md). Design: [eval/strategy/github-app-skeleton-2026-05.md](./eval/strategy/github-app-skeleton-2026-05.md).
